...
```
workflow-starter/
├── AGENTS.md                # 本文件（22 条硬规则 + 索引）
├── README.md                # 怎么用：4 步 setup + 飞轮机制说明
├── .gitignore               # 排除密码/凭证/临时文件
├── 拉起Codex.bat           # 快速启动 Codex（跳过权限确认）
├── 启动官方Codex.bat       # 带代理启动 Codex
├── docs/
...
## 可用 Skills（`vllm-omni/.Codex/skills/`）

仓库内置的接入工程模板，由 Codex 自动加载。**何时主动调用**：
...
**调用方式**：用 `Skill` 工具，`skill` 字段填名字（如 `add-diffusion-model`）。**不要**和 `AGENTS.md` 里的硬规则、`memory/`、`.claude_errors/` 冲突——skill 是"怎么写代码 fit 进框架"，硬规则是"怎么不踩远端/容器/调试的坑"，两者互补。
