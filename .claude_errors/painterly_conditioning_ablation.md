...
**对未来的提醒**：painterly 已确认**不在 vllm-omni 这 45 个 main commit 范围内**。下次别再花时间 `git log origin/main..HEAD` 找精度修复 commit。bug 必然在：(a) cr/pr3107-fix 自己的 21 个适配 commit，(b) vllm 0.20.0 / torch 2.11 / cuDNN 13 这些非 vllm-omni 组件，(c) 或 vllm-omni AR/DiT 跨进程架构本身。Branch 已 push 到 `<GH_USER>/vllm-omni:cr/pr3107-rebased` 备查。
