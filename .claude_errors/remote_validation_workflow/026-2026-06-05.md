...
第一次 graph run 在 engine READY 前失败，根因是 worker 子进程找不到 `ninja`。我发现 `/home/yueqian/.venv-v022rc1/bin/ninja` 存在后，只补了 `PATH` 就完整重跑 160GB 模型。第二次 run 超过 20 分钟未产生 profiler artifact，用户指出“不符合预期”。复查进程树后才确认真正卡点是：
