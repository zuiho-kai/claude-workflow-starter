...
- `--chat-template hunyuan_image3_i2t.jinja` 使用相对路径，vLLM 校验 path-like 字符串时找不到文件。正确路径是 `/home/wzr/vllm-omni/hunyuan_image3_i2t.jinja`。
...
- 真正的最终 blocker 是 FlashInfer sampling JIT 找不到 `ninja` 可执行文件：`ninja` 包在 `/home/wzr/.venv` 里已安装，但 serve 进程 PATH 没有 `/home/wzr/.venv/bin`，导致 warmup 阶段 `FileNotFoundError: [Errno 2] No such file or directory: 'ninja'`。
