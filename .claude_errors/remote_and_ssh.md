...
**症状**：CI 报 `ruff check` 的 F841 未使用变量，本地删除变量后已经跑过覆盖改动文件的 `ruff check` 和 `py_compile`，但我又默认 SSH 到 `$WORK_DIR/wt-hunyuan-it2i-ar-stream` 远端复验，结果远端 venv 缺 ruff binary，白白引入环境噪音。
...
**症状**：用户在新节点（`<USER>@<HOST>:<PORT>`）要求"new own venv"，我没用 `uv venv` 而是 `cp -r $DATA_DIR/venv .venv`，8.8GB 源拷成 25GB+ 还没生成 `bin/python`，半天动不了。换 `cp -rP` 重试仍然异常膨胀
...
```bash
cd /tmp
uv venv --python /usr/bin/python3 --system-site-packages $WORK_DIR/wt-i2t-test-fix/.venv
uv pip install --python $WORK_DIR/wt-i2t-test-fix/.venv/bin/python --no-deps -U 'vllm==0.21.0'
```
然后验证：
```python
...
```bash
cd /tmp
uv pip install --python $WORK_DIR/wt-i2t-test-fix/.venv/bin/python --torch-backend cu130 \
...
```bash
uv pip uninstall --python $WORK_DIR/wt-i2t-test-fix/.venv/bin/python torch torchaudio torchvision triton
```
再确认：
```text
torch 2.11.0+cu130
...
4. 用户明确说"新建 venv，把旧的干掉"时，只删除当前 worktree 自己的 `.venv`；删除前 `readlink -f` 确认路径在 worktree 内，绝不碰共享 `$WORK_DIR/vllm-omni/.venv`。
...
**症状**：`gh pr edit 3626 --body-file ...` 报 `Failed to log in to github.com account <GH_ORG> (keyring) - The token in keyring is invalid`。
...
**症状**：用户给 `ssh <USER>@<HOST> -p <PORT>` 让我验证，我直接连上后扫 `python --version` / `find` 环境，报默认 `python` 不存在，然后才被用户指出 `docs/remote_server.md` 已写明 `$WORK_DIR` 下的工作目录和 venv。
...
```
然后以 `$WORK_DIR` 为稳定锚点。用户明确预期是：**本地修改先 commit + push 到远程分支；服务器在 `$WORK_DIR` 下新建本次专用 worktree 和 venv；远端通过 git fetch/pull 同步代码后验证**。不能把某个旧 `wt-*` 或 `.venv` 当永久路径（用户会不定期删除 worktree 和 venv）。

先动态查看当前 `$WORK_DIR`，避免命名冲突：
```bash
ls -la $WORK_DIR
find $WORK_DIR -maxdepth 3 -type f \( -name pyvenv.cfg -o -name activate \) 2>/dev/null | sort
find $WORK_DIR -maxdepth 2 -type d -name "wt-*" 2>/dev/null | sort
```
再新建本次专用目录（名字按任务改）：
```bash
cd $WORK_DIR
git clone <repo-url> wt-<task>   # 若已有合适 repo，可 git fetch 后 git worktree add $WORK_DIR/wt-<task> <branch>
cd $WORK_DIR/wt-<task>
...
2. `$WORK_DIR` 是稳定目录；每次远端验证默认在 `$WORK_DIR` 下新建本次专用 worktree 和 `.venv`，不要复用可能被删除的旧路径。
...
```bash
find $WORK_DIR -maxdepth 4 \( -name "*hunyuan*yaml" -o -name "deploy*.yaml" -o -name "*image3*.yaml" \) -print
...
3. 先做 import/init smoke：
   ```bash
   PYTHONPATH=$WORK_DIR/wt-pr-base $VENV/bin/python - <<'PY'
   import vllm
...
**症状**：PR #3606 分支已经强推到 `bddc12a5a`，远端也已有 `$WORK_DIR/wt-pr3606-base` 和 `$WORK_DIR/wt-pr3606-patch` 两个现成 worktree。但新终端上来后先跑了大量探测命令（find repo、find venv、grep profiler、grep Omni、查 deploy yaml、查历史脚本等），中间还因为 PowerShell 管道 CRLF 误判文档路径不存在。最后才复用现成 base/patch 目录并启动 tmux 长跑测试。
