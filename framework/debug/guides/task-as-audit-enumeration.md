...
**Why:** Task list 本身不消除偏见——偏见在生成 task 那一步。把外部 review 的 action list 直接当 task list 等于把"找问题"完全外包给一个有 framing 的子 agent（见 [review_delegation_framing](../../agents/guides/review-delegation-framing.md)），它的盲点 100% 继承。task 列在**还不知道答案**之前开，才能逼自己走完审查面。
