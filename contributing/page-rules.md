...
新增或实质扩张 `repos/<仓库>/rules.md` 前，必须先回答：

1. 做这个仓库里另一类无关任务的人，是否仍必须默认读到这条规则？
2. 是否找不到一个更窄且稳定的工作主题、component 或 model owner？
3. 正文是否只包含默认硬约束，而不包含模板、逐步方法、例子或专项测试矩阵？

只有三个答案都为“是”，正文才进入仓库根 `rules.md`。否则把硬约束放进最近 owner 的 `rules.md`，把模板和操作方法放进该 owner 的 `guides/`，仓库根只保留一行场景路由。不能因为结论“只适用于这个仓库”就直接选择仓库根，也不能等根规则写长后才按行数拆分。

规则必须脱离单次 issue 也能读懂，不要求人先知道事故编号。

写入前做内容审计，不按“这次复盘有哪些材料”整包落盘：

- 可复用的必须、禁止和验收条件留在 `rules.md`；
- 稳定职责、边界和数据流移到 `architecture.md`；
- 非硬门禁但可复用的方法移到 `guides/`；
- 日期、PR/SHA、一次性测试数量和调查时间线默认不保存，追溯交给 Git/PR 历史。

不得为容纳被规则审计剔除的历史细节而自动新建 incident。incident 是否准入只按 [复盘与错题](incidents.md) 的三项门禁判断。
