...
向 `zuiho-kai/claude-workflow-starter` 这个公开上游提交 commit 时使用 DCO sign-off（`git commit -s`）。第三方把框架接入自己的仓库后，应按自己的贡献政策保留、修改或删除这条上游专属要求；不要从其他 `repos/<仓库>/` 继承 Git 身份和 remote。
...
规则是默认执行面，不是事故摘要。能改变下一次行为、提前阻止错误或定义最低验收的结论，优先写进离根因最近的 `rules.md`：

- 多个仓库都成立，写对应 `framework/<主题>/` 的规则；
- 整个仓库都成立，写 `repos/<仓库>/rules.md`；
- 只属于稳定的共享源码模块，写 `components/<模块>/rules.md`；
- 只属于某个模型，写 `models/<模型>/rules.md`。

不要预建空 `rules.md`。只有至少有一条可执行规则时才创建，并从同目录 `_index.md` 链接。规则至少写清触发条件、必须做什么、禁止什么和怎样验收；不要要求读者先知道某个 issue 编号或事故文件名。

默认查询顺序为：通用方法 → 当前仓库规则 → 必要的仓库主题 → `components/_index.md` 或 `models/_index.md` 的职责地图 → 已确认 owner 的规则。根入口的场景触发器已经直接指向具体 guide 时，不再先读该主题索引。canonical `repos/<slug>/` 已在本轮验证时可直读仓库规则；用户只给 upstream、URL、显示名或本地目录时，先用 `repos/_index.md` 映射，不自行拼 slug。仓库同名主题由任务目的决定，不因通用 guide 恰好放在某个物理目录就自动进入。先选一个最可能制造错误状态的主要 owner，读完后停止横向展开；只有 live 调用链证明错误跨越另一个模块边界时才读取第二个模块，不能为了保险遍历所有看起来相关的目录。没有现成 owner 时先按 live 源码调查，复盘确认边界稳定后再更新职责地图；不能因为一次错误就创建新模块。已有完整日志和可读源码的窄 bug 优先按仓库规则完成诊断路由，`code taste` 在真正编辑前再读。
...
用户要求复盘、沉淀经验或总结教训时，先完成四件事：

1. 用 live 证据说明为什么发生；
2. 说明为什么原有规则、校验、测试或路由没有发现；
3. 把怎样提前阻止写进最近 owner 的 `rules.md`；
4. 写清最小验收，证明新规则真的能拦住同类问题。

错题不是必选产物。只有根因链、复现条件、日志证据或历史背景很复杂，规则无法承载且未来仍有独立查询价值时，才增加 `incidents/` 页面。正常开工和查询必须从 `_index.md`、`rules.md` 和职责地图进入，不能要求人或 agent 先猜事故文件路径。
...
根因还没查清时默认先继续调查，不急着创建错题。用户明确要求保留调查记录时，才暂放最接近的仓库主题并标明“待归类”；查清后移动到最终位置，并在同一次修改中更新所有链接。
...
新增错题前必须先更新或确认对应规则。错题末尾增加“已提炼到”链接；规则页面不复制整篇事故经过，也不能反过来要求正常任务必须读取错题。
...
- 检查目录和页面是否进入最近的 `_index.md`；
- 检查相对链接、错题字段、重复编号和大小阈值；
- 提醒 `local/` 被跟踪或正式页面出现明显敏感信息。
...
3. 如果新增、删除或重命名 `framework/` 的一级主题，同步更新 `framework/_index.md`、`CLAUDE.md` 和 `README.md` 中的任务地图。
4. 修复所有指向旧路径和新路径的相对链接。
5. 移动错题时保持“编号”不变，只修改“归属”和路径。
6. 删除已经被新页面替代的重复正文。
7. 运行检查脚本，确认没有漏进索引的页面和断开的链接。
...
1. 先确认当前仓库。
2. 根据任务选择一个通用主题；场景触发器直接指向 guide 时跳过主题索引。
3. canonical `repos/<slug>/` 已验证时先读仓库 `rules.md`；用户只给 upstream、URL、显示名或本地目录时，先用 `repos/_index.md` 映射。规则命中 owner 就停止导航，未命中才读仓库 `_index.md` 和一个主要主题。仓库主题按任务目的选，不按通用 guide 的物理目录选。
4. 涉及源码时先从 `components/_index.md` 或 `models/_index.md` 的职责地图选择一个主要 owner，再读取对应目录；选定后停止横向展开，除非 live 调用链证明问题跨模块。不要按单次错误名称猜目录，也不要保险式遍历。
5. 历史错题只在规则明确提示、出现高度相似错误或用户明确调查历史时搜索。
...
- 新的通用经验只写 `framework/`。
- 新的仓库、代码模块和模型知识只写 `repos/`；稳定教训先写最近 owner 的规则，复杂事故证据才可选写错题。
...
- 确需保留复杂事故证据时，diffusion 共享事故进入 `components/diffusion/incidents/`，模型专有事故进入对应模型目录；稳定结论已经写入规则。
- 前端、后端等代码模块与 dev、review、CI 等工作主题平级，不互相套娃。
- 每次复盘已经把可执行结论写入最近 owner 的规则；确有必要的错题只有一份完整正文，并链接提炼后的规则。
