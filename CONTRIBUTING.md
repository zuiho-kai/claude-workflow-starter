...
查询顺序固定为：通用主题 → 当前仓库同主题和仓库规则 → `components/_index.md` 或 `models/_index.md` 的职责地图 → 已确认 owner 的规则。先选一个最可能制造错误状态的主要 owner，读完后停止横向展开；只有 live 调用链证明错误跨越另一个模块边界时才读取第二个模块，不能为了保险遍历所有看起来相关的目录。没有现成 owner 时先按 live 源码调查，复盘确认边界稳定后再更新职责地图；不能因为一次错误就创建新模块。
...
1. 先确认当前仓库。
2. 根据任务选择一个通用主题。
3. 当前仓库已登记时，必须读取仓库 `_index.md`、`rules.md` 和同名仓库主题，不能停在通用层。
