...
查询顺序固定为：通用主题 → 当前仓库同主题和仓库规则 → `components/_index.md` 或 `models/_index.md` 的职责地图 → 已确认 owner 的规则。没有现成 owner 时先按 live 源码调查，复盘确认边界稳定后再更新职责地图；不能因为一次错误就创建新模块。
...
4. 涉及源码时先从 `components/_index.md` 或 `models/_index.md` 的职责地图选择已有 owner，再读取对应目录；不要按单次错误名称猜目录。
