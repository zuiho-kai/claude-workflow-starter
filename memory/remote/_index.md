...
| [live_env_source_of_truth.md](live_env_source_of_truth.md) | 远端模型/cache/venv/GPU 路径必须以当前机器 live env 或正在跑的进程为准；旧 memory 只能当线索，不能直接变成 `HF_HOME` / model path |
| [container_setup.md](container_setup.md) | 容器持久化：HF cache `unset TRANSFORMERS_CACHE/HF_HUB_CACHE` 必须做（空字符串 ≠ unset）；uv/vLLM/venv/cache 可只读复用已有 `/root`，缺失时禁补装到 `/root` 或 `/root/.cache`；所有产出写挂载 Lustre 路径；docker exec chdir 报错先 cd 匹配宿主路径；multiprocessing 前 `cd /tmp` |
