...
| [container_setup.md](container_setup.md) | 容器持久化：HF cache `unset TRANSFORMERS_CACHE/HF_HUB_CACHE` 必须做（空字符串 ≠ unset）；所有产出写挂载持久路径；multiprocessing 前 `cd /tmp` |
...
| [hf_offline_mandatory.md](hf_offline_mandatory.md) | 远端 HF 加载（`from_pretrained()` / 框架 init）必须 `HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1`；只设 `HF_HOME` 不够，hub 仍校验 revision 触发重下 |
