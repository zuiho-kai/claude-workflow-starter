...
description: 远端跑任何 HF 模型加载前必须 export HF_HUB_OFFLINE=1 + TRANSFORMERS_OFFLINE=1；只设 HF_HOME 不够，hub 仍会校验 revision / 拉缺失 shard，160GB 模型一启动就把网络+磁盘打爆
type: rule
---
...
- 2026-05-14 /data/wzr 新建 venv 跑 profile_ar.py，只设 `HF_HOME=/data/models` 没设 offline，模型疑似触发重新下载 → 磁盘 IO + 网络拉满 → SSH `kex_exchange_identification: Connection closed by remote host`，5 分钟内连不上服务器
...
- 类似坑参见 [container_setup](container_setup.md) §2（cache 路径覆盖）

## 相关

- [container_setup](container_setup.md) §2：`TRANSFORMERS_CACHE` / `HF_HUB_CACHE` 优先级覆盖 `HF_HOME`（同根问题：cache 配置不到位）
- CLAUDE.md A11
