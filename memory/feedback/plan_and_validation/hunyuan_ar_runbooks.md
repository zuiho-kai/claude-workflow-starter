# Plan & Validation · AR graph / profiling 机制

本页是高频机制层，只保留诊断漏斗、scope lock、证据等级和停止条件。HunyuanImage3 AR-only / TP2 / PR #3767 的具体 host、artifact、历史结果和复跑命令已经下沉到 [archive/hunyuan/ar_only_tp2_benchmark_runbook.md](../../archive/hunyuan/ar_only_tp2_benchmark_runbook.md)。

## 机制入口

命中以下任一场景时读本页：

- 用户给 profiler / Perfetto / Chrome trace 截图，要求判断 GPU idle、step gap、copy gap 或 graph replay 是否能优化。
- 用户要求远端跑性能 / 精度验证，而且希望复用已有 benchmark、smoke 或 profiling 脚本。
- 用户问 graph mode / profiler / steady-state latency 的结论是否可写进 PR。

先执行三条硬规则：

1. 先写 scope lock，再开 GPU 长跑。
2. 先低开销 probe / no-profiler benchmark，再 full torch trace。
3. profiler 只解释热点；正式性能结论必须来自 no-profiler steady-state run。
...
- 如果 Hunyuan AR graph online 的 input-prep 周期性长空洞复现，必须加慢 `.item()` 栈采样：`AR_DECODE_DIAG_WRAP_TENSOR_ITEM=1 AR_DECODE_DIAG_ITEM_STACK_MS=50`。2026-06-11 的 np3 栈证明显示，请求窗口内 `torch.Tensor.item` 慢记录全部命中 `/home/wzr/vllm-omni/vllm_omni/model_executor/models/hunyuan_image3/hunyuan_image3.py:2011` 的 `n_timestep = int(timestep_mask.sum().item())`，单次约 `198-205ms`，属于 CUDA scalar sync；这类结论必须有 stack proof，不能只凭 Perfetto 的 `cudaMemcpyAsync -> HtoD` 空白判断。
