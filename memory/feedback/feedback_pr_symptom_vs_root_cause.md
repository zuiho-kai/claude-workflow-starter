...
**典型反例**：某 PR 只修了下游 `_map_device_list` 的映射逻辑（symptom），没发现真正的根因是上游 `_initialize_stages` 里的锁是 per-instance 的，导致多实例并发时 `CUDA_VISIBLE_DEVICES` 被污染，才传入了"错误的" visible 列表。

**How to apply:** 看到"环境变量读-改-还原"模式 + 多实例并发，立刻检查锁是模块级还是实例级；reviewer 反驳 fallback 修复无效，先回上游找谁污染了 input 再继续。
