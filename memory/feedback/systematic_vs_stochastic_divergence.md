...
description: When two code paths produce different outputs on the same model, the FIRST move is to classify "systematic" vs "stochastic". I once spent hours on CUDA/MoE non-determinism handwaving when the real cause was a PIL Image alpha-channel mismatch upstream of the model. Don't repeat.
type: feedback
---
...
- `memory/feedback/seed_determinism_audit.md` (previous P2 doc — the conclusion there was correct as far as it went, but I treated it as the whole answer when it was only half)
