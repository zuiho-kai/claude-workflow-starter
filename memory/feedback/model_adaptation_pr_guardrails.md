...
description: 新模型 / 新 pipeline 适配 PR 的开发前 gate、公开入口矩阵、验证证据口径和交互式 reviewer loop。用于避免 checkpoint 结构误判、public docs 不可运行、request parsing 漂移、性能/测试结果过 claim。
type: feedback
---
...
**硬规则**：开发前先写 mini spec。sub-agent review 只能兜底，不能替代开发前 gate。能跑通 unit test 不等于 public entrypoint 可用，不等于 PR body 可以写性能结论。声明 PR clean 前必须跑 full diff review；关闭已知 finding 不等于全量 review 干净。

## 1. 开发前 mini spec

写代码前必须先列这张短矩阵，允许很短，但不能空：
...
公开 PR body / comment 禁止写：

- 本地 Windows 缺依赖 blocker
- 远端 host/user/path
- cache 路径、cache miss 细节
- "某机器没找到模型缓存" 这类私有环境事实
...
- pass/fail outcome
- "live serving smoke pending" 这类公开边界
