...
description: trust_remote_code 模型接入时反复换参数试错的反思——查根因 / 读 requirements.txt / 改 snapshot 不改 cache
type: feedback
---

## 1. 先读官方 requirements.txt，不要猜版本

connector 包（如 transformers）的版本兼容性踩坑：4.50 → 5.6.2 → 4.57.1 试了三轮才发现官方 requirements.txt 写的是 4.57.1。每次装错版本 = 重建 venv + 重新加载模型（各 5 分钟），三轮就是 30 分钟。
**How to apply:** 跑任何 HF 模型的官方 demo 前，第一步 `curl` 他们的 `requirements.txt`，用精确版本。

## 2. trust_remote_code 模型不要假设标准参数生效

`attn_implementation="eager"` 传了但模型自定义了 `XX_ATTENTION_CLASSES` 硬编码只有 SDPA。试了多轮 eager 都没用。
...
用户要 torch profiler trace（时序图 JSON），我跑了 benchmark stats JSON。浪费一轮完整的 benchmark 时间。
...
## 6. trust_remote_code 模型 patch 必须改 snapshot，不能改 cache
...
## 7. 有 runbook 就直接用 runbook 的版本，任何"先试别的"都是浪费

如果团队/项目已经有验证过的 transformers + 模型版本 runbook，**直接用**。偏离一个版本 = 引入任意多个 API 变化。
**How to apply:** 进远端第一步 `grep transformers requirements.txt`，对上了再跑。给不同模型/任务建专用 venv（如 `venv_hf` = 官方 baseline 专用、`venv` = 自己框架专用），别混用。
