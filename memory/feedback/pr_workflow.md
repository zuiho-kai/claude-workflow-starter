...
- **Sign-off identity**: `Signed-off-by: <YOUR_GIT_NAME> <YOUR_GIT_EMAIL>`. 节点上 `git config user.{name,email}` 应该已经配好——`git commit -s` 自动带上。如果哪天发现 commit 实际 sign-off 用了别的邮箱（例如新节点 git config 没设），就 `git -c user.email=<YOUR_GIT_EMAIL> -c user.name=<YOUR_GIT_NAME> commit --amend -s` 改回，再 force-with-lease 推。
...
```bash
ssh ... 'cd $WORK_DIR/wt-<task> && git format-patch -1 --stdout HEAD > /tmp/fix.patch'
