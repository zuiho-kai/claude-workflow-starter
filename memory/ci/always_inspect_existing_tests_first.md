...
在 B3 `root@106.15.124.84 -p 31449` 上跑 PR 分支的可复用环境：

```bash
cd /data/wzr/wt-pr3297-accuracy
...
export PYTHONPATH=/data/wzr/pydeps-pr3297-accuracy:/data/wzr/wt-pr3297-accuracy:${PYTHONPATH:-}
export TMPDIR=/tmp

/data/ngngaifai/.venv/bin/python3 -m pytest -s -v \
  tests/e2e/accuracy/test_hunyuan_image3.py::test_image_to_image_alignment \
  --tb=short
```

运行前要做硬 gate：选中的 4 张物理 GPU 都低于 5 GiB，并连续检查几轮。这个测试里 Stage 0 AR 用可见卡里的逻辑 `0,1`，Stage 1 DiT 用逻辑 `2,3`；例如 `CUDA_VISIBLE_DEVICES=4,5,6,7` 时，AR 落物理 4/5，DiT 落物理 6/7。共享节点会在 gate 之后仍被其他任务抢占，失败时先用 `/proc/<pid>/cwd` 确认进程归属，只清理本次 `/data/wzr/wt-pr3297-accuracy` 进程组，不能误杀其他人的 `/data/why` 或 `/data/dyy` 任务。

踩坑记录：

- `/data/why/.venv` 的 vLLM 0.22 会和 PR #3297 代码 ABI 不匹配，报 `split_routed_experts` import error；用 `/data/ngngaifai/.venv/bin/python3`（vLLM 0.21.0）。
- target deps 放 `/data/wzr/pydeps-pr3297-accuracy`，至少需要 `FlagEmbedding`、`datasets`、`torchmetrics`；避免覆盖 venv 自带的 `numpy` / `huggingface_hub` 等核心包。
- B3 没有 `cublasLt.h`，默认 FlashInfer MoE JIT 会失败；必须显式 `VLLM_USE_FLASHINFER_MOE_FP16=0` 走 TRITON MoE。
- 不要把 `TMPDIR` 指到 `/data/wzr/tmp`，ZMQ IPC 会报 `Operation not supported (addr='ipc:///data/wzr/tmp/...')`；用 `/tmp`。
...
```text
/data/wzr/pr3297_accuracy_v21_no_fi_gate6_20260602_193332
```
...
```text
/data/wzr/pr3297_accuracy_stage0_mm_sdpa_patch3_20260604_113531
```
