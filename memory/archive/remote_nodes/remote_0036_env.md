...
description: hk01dgx036 节点环境和 0006 完全不同，路径结构和挂载都要改
type: reference
---

# hk01dgx036 节点特性（和 0006 不一样）
...
- srun：`srun -p q-fq9hpsac -w hk01dgx036 --gres=gpu:2 --cpus-per-gpu=24 --mem-per-cpu=8G --job-name=wzr --pty bash`
...
```bash
docker run -d --name wzr_omni --gpus all --ipc=host --network=host \
