...
**何时来翻**：只在需要复用 2026-06-08 HunyuanImage3 AR graph serve / online profiler 的具体远端结果、artifact、路径或错误签名时读。通用机制见 `memory/feedback/remote_debug_strategy/ar_graph_profiling.md`。
...
```text
remote: root@106.15.124.84 -p 31342
...
1. 相对 chat template 路径先把服务挡在入口外；HunyuanImage3 AR serve 一律用绝对路径：`/home/wzr/vllm-omni/hunyuan_image3_i2t.jinja`。
...
5. 最终 blocker 是 PATH 不是包缺失；`ninja` 包已安装在 `/home/wzr/.venv`，但 serve 进程 PATH 没有 `/home/wzr/.venv/bin`。

启动前 gate：

```bash
export PATH=/home/wzr/.venv/bin:$PATH
command -v ninja
ninja --version
```
...
/home/wzr/.venv/bin/vllm serve tencent/HunyuanImage-3.0-Instruct \
  --omni \
  --port 8091 \
  --trust-remote-code \
  --deploy-config /tmp/<run>/deploy_config.yaml \
  --chat-template /home/wzr/vllm-omni/hunyuan_image3_i2t.jinja \
