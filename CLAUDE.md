...
窄 bug 快路径：用户已经明确仓库名时，直接检查约定路径 `repos/<仓库>/_index.md`，存在就读取该仓库规则，不再先读 `repos/_index.md`；仓库场景触发器又直接命中 owner 时，直接进入 owner 规则，不再读取通用 debug 索引或仓库 debug fallback。任一直接入口不存在或没有匹配时，立即回到标准“通用主题 → 仓库主题 → 职责地图”路由。
